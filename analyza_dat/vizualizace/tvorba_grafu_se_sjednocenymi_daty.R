...
ggplot(data_daily, aes(x = den, y = prumer_pm10)) +
...
ggplot(data_daily, aes(x = den, y = prumer_no2)) +
  geom_line(color = "grey30", linewidth = 0.7) +
  scale_x_date(
    date_breaks = "1 month",
    date_labels = "%m/%Y"
...
###################################################################### 
##                                  
##       podmínky počasí (data)    
##                                  
###################################################################### 

cor_matrix <- cor(
  data[, c("data_windSpeed", "data_windImpact")],
...
deskriptiva_wind <- psych::describe(data[, weather_cols])
...
summary(data$valid_speed_count)
psych::describe(data$valid_speed_count)
