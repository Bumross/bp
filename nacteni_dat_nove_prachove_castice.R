...
polution_summary <- polution %>%
  ...
  group_by(sensor_id, hour) %>%
  summarise(
    sum_pm10 = mean(data_pm10, na.rm = TRUE),
    sum_pm25 = mean(data_pm25, na.rm = TRUE),
    sum_pm40 = mean(data_pm40, na.rm = TRUE),
    sum_pm100 = mean(data_pm100, na.rm = TRUE),
...
hourly_summary <- polution_summary %>%
  group_by(hour) %>%
  summarise(
    total_pm10 = mean(sum_pm10, na.rm = TRUE),
    total_pm25 = mean(sum_pm25, na.rm = TRUE),
    total_pm40 = mean(sum_pm40, na.rm = TRUE),
    total_pm100 = mean(sum_pm100, na.rm = TRUE),
    avg_no2 = mean(avg_no2, na.rm = TRUE),
    .groups = "drop"
  )


pm_data <- hourly_summary %>%
  select(total_pm10, total_pm25, total_pm40, total_pm100)


cor_matrix <- cor(pm_data, use = "complete.obs", method = "pearson")
